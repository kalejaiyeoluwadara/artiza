...
- **The register** — the full list, headed "Hands in Ilisan" (a trade filter narrows the same sentence: "Tilers in Ilisan"). It shows **one page of six**, with the count on the right turned into the control that reveals the rest: "View all 24" → "Show fewer". The rails above already argue for who's worth seeing first, so the list is a browse surface, not an index.
