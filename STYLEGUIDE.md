...
- **Home runs a wider column than the rest of the app** — `max-w-[96rem]` with `md:px-8 lg:px-12`, against `max-w-5xl` everywhere else. A catalogue of rails wants the screen; a settings list or a search result does not. The shared desktop header matches whichever column it is sitting above, or the logo ends up visibly inboard of the artwork it heads.
- **Desktop is not mobile stretched.** Three things change at `md` and they are the whole difference: the billboard's type moves from centred-under-the-art to a left column beside it (with a second left-to-right scrim so the right half of the photo stays unclouded), posters step up a size again, and rails grow hover arrows. A rail on a touch screen is flicked and needs no affordance; a mouse cannot flick, so the arrows are the desktop equivalent of the peeking next poster.
- **Rail arrows** appear only where there is something to scroll to — the rail reads its own `scrollLeft` and hides the arrow at each end, since an arrow pointing at nothing is worse than no arrow. They fade in with the row on hover rather than sitting there, so a rail at rest is all artwork.
- **Hover belongs to `.hover-lift`, `.hover-fill`, `.hover-dim`** — all three live behind `@media (hover: hover) and (pointer: fine)` in `globals.css`. Raw Tailwind `hover:` is not gated and leaves a tapped element stuck in its hover state on a phone; don't reach for it. A poster lifts as one tile, wrapper and heart together — lifting the button alone slides the artwork out from under a heart that stayed put.
...
- **Artisan page** — `/artisan/[slug]`, where a shared link lands, and deliberately not a second design: the body is the very same `ArtisanProfile` the sheet renders, at the sheet's `max-w-lg` column rather than the app's `max-w-5xl`, with the sheet's chrome translated to a page (a way back where the grabber was, the unlock module stuck to the viewport clearing the tab bar). The URL is the artisan's name, never an id — a link gets pasted into a sentence in a WhatsApp chat, and a UUID in the middle of that sentence reads as spam. `lib/slug.ts` owns the mapping both ways.
- **Share mark** — the one custom icon outside `TabIcons`, drawn to the same grid in `components/ShareButton.tsx` because it is a motion component rather than a static glyph: three nodes and two *curved* arcs (every stock share icon draws them straight — the curve is the character), the far nodes travelling outward on hover with the arcs animating their own `d` to follow, and a check drawn in one stroke on success. It sits beside the artisan's name, never near the unlock button: passing an artisan on is free, and it must not be mistaken for the paid action. It copies the link and toasts, on every surface — it does **not** call the Web Share API, which desktop browsers also advertise and answer with an OS menu of AirDrop, Notes and Reminders drawn over the profile being shared.
